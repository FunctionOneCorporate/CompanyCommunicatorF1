...
namespace Microsoft.Teams.Apps.CompanyCommunicator.DraftNotificationPreview
{
    ...
    public class DraftNotificationPreviewService
    {
        ...
        private readonly string webApiLog;
        ...
        public DraftNotificationPreviewService(
            ...
            IOptions<DraftNotificationPreviewOptions> options)
        {
            ...
            this.webApiLog = options.Value.UrlApiLog;
            if (string.IsNullOrEmpty(this.webApiLog))
            {
                throw new ApplicationException("UrlApiLog setting is missing in the configuration.");
            }
        ...
        {
            var recipient = turnContext.Activity.Recipient.Id;
            var reply = this.CreateReply(draftNotificationEntity, recipient);

            await turnContext.SendActivityAsync(reply);
        }

        private IMessageActivity CreateReply(NotificationDataEntity draftNotificationEntity, string recipient)
        {
            var adaptiveCard = JsonConvert.SerializeObject(this.adaptiveCardCreator.CreateAdaptiveCard(
                ...
                draftNotificationEntity.HeaderImgLink,
                draftNotificationEntity.FooterImgLink));

            string url = this.webApiLog + "?notificationId=" + draftNotificationEntity.Id + "&aadid=" + recipient;
            var msg = JsonConvert.DeserializeObject(adaptiveCard.Replace("https://f1.com.br/log", url));
