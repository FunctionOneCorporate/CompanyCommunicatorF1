	...
	public render(): JSX.Element {
		...
		const processItem = (message: any) => {
			...
			const out = {
				...
				onClick: (): void => {
					let url =
						getBaseUrl() + "/viewstatus/" + message.id + "?locale={locale}";
		...
		if (this.state.loader) {
			...
			return (
				<div className="results">{this.localize("EmptySentMessages")}</div>
			);
		} else {
		...
		return (
			<Flex className="listContainer" vAlign="center" fill gap="gap.small">
				<Flex.Item
					...
					variables={{ "size.quarter": "24%" }}
					grow={1}>
					<Text truncated content={isStringNullOrWhiteSpace(message.title) ? message.nmMensagem : message.title}></Text>
				</Flex.Item>
				...
				<Flex.Item
					...
					<div>
						<TooltipHost
							content={this.props.t("TooltipSuccess")}
							calloutProps={{ gapSpace: 0 }}>
							<Icon
								name="stardust-checkmark"
								xSpacing="after"
								className="succeeded"
								outline
							/>
							<span className="semiBold">
								{formatNumber(message.succeeded)}
							</span>
						</TooltipHost>
						<TooltipHost
							content={this.props.t("TooltipFailure")}
							calloutProps={{ gapSpace: 0 }}>
							<Icon
								name="stardust-close"
								xSpacing="both"
