// <copyright file="TemplateNotification.cs" company="Microsoft">
...
namespace Microsoft.Teams.Apps.CompanyCommunicator.Models
{
    ...
    /// <summary>
    /// Template notification model class.
    /// </summary>
    public class TemplateNotification : BaseNotification
    {
        private static readonly int MaxSelectedTeamNum = 20;

        /// <summary>
        /// Initializes a new instance of the <see cref="TemplateNotification"/> class.
        /// </summary>
        ...
        public bool Validate(IStringLocalizer<Strings> localizer, out string errorMessage)
        {
            ...
            if (teams.Count > TemplateNotification.MaxSelectedTeamNum)
            {
                var format = localizer.GetString("NumberOfTeamsExceededLimitWarningFormat");
                stringBuilder.AppendFormat(format, teams.Count, TemplateNotification.MaxSelectedTeamNum);
                stringBuilder.AppendLine();
            }

            var rosters = this.Rosters.ToList();
            if (rosters.Count > TemplateNotification.MaxSelectedTeamNum)
            {
                var format = localizer.GetString("NumberOfRostersExceededLimitWarningFormat");
                stringBuilder.AppendFormat(format, rosters.Count, TemplateNotification.MaxSelectedTeamNum);
                stringBuilder.AppendLine();
            }

            var groups = this.Groups.ToList();
            if (groups.Count > TemplateNotification.MaxSelectedTeamNum)
            {
                var format = localizer.GetString("NumberOfGroupsExceededLimitWarningFormat");
                stringBuilder.AppendFormat(format, groups.Count, TemplateNotification.MaxSelectedTeamNum);
                stringBuilder.AppendLine();
            }
