    ...
    public class SendFunction
    {
            ...
            ILogger log)
        {
            ...
            await this.notificationService.UpdateSentNotification(
                    ...
                    errorMessage: sendMessageResponse.ErrorMessage,
                    messageId: sendMessageResponse.MessageTeamsId);
        ...
        {
            string url = this.webApiLog + "?notificationId=" + message.NotificationId + "&aadid=" + message.RecipientData.RecipientId;
            var notification = await this.notificationRepo.GetAsync(
                NotificationDataTableNames.SendingNotificationsPartition,
                message.NotificationId);

            var msg = notification.Content.Replace("https://f1.com.br/log", url);

            var adaptiveCardAttachment = new Attachment()
            {
                ContentType = AdaptiveCardContentType,
                Content = JsonConvert.DeserializeObject(msg),
            };
